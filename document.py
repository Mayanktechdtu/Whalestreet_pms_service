...
''', unsafe_allow_html=True)
    ...
    aggregate_data = {
        ...
        "Total Return": ["₹3,63,000", "₹2,83,750", "₹1,15,000"],  # Updated total return
    ...
    if model_choice == 'Complex Model with Management Fees and Profit Share Threshold':
        ...
        def calculate_min_return_to_beat_fd():
            for r in np.arange(6.5, 100.0, 0.01):
                management_fee = r * (management_fee_percentage / 100)
                profit_share = max(0, (r - 8.0) * (profit_share_percentage / 100))
                net_return = r - management_fee - profit_share
                if net_return >= 6.5:
                    return r

        min_return_to_beat_fd = calculate_min_return_to_beat_fd()

        def calculate_min_return_for_15_net():
            for r in np.arange(15.0, 100.0, 0.01):
                management_fee = r * (management_fee_percentage / 100)
                profit_share = max(0, (r - 8.0) * (profit_share_percentage / 100))
                net_return = r - management_fee - profit_share
                if net_return >= 15.0:
        ...
        min_return_for_15_net = calculate_min_return_for_15_net()

        st.markdown(f"**Minimum % Return Required to Beat FD Returns (6.5%)**: {min_return_to_beat_fd:.2f}%")
        st.markdown(f"**Minimum % Return Required to Achieve 15% Net Return on Capital**: {min_return_for_15_net:.2f}%")
        ...
        for i in years:
            ...
            management_fee = management_fee_percentage / 100 * total_capital
            capital_after_management_fee = total_capital - management_fee

            if total_return_value / capitals[-1] * 100 > 8.0:
                profit_share = profit_share_percentage / 100 * (total_return_value - (8.0 / 100 * capitals[-1]))
            else:
                profit_share = 0
        ...
        df_complex = pd.DataFrame({
            'Year': years,
            'Initial Capital (₹)': [initial_capital] + net_capital_after_fees[:-1],
            'Total Capital': capitals[1:], 
            'Net Capital After Fees': net_capital_after_fees, 
            f'{management_fee_percentage}% Management Fee (₹)': management_fees,
            '20% Profit Share Above 8% (₹)': profit_shares,
            '% Gain Net': net_gains
        })
        ...
        fig_complex.add_trace(go.Bar(
            ...
            text=[f'₹{df_complex["Net Capital After Fees"][i]:,.2f}' for i in range(len(df_complex))],
        ...
        fig_complex.add_trace(go.Bar(
            ...
            text=[f'₹{df_complex[f"{management_fee_percentage}% Management Fee (₹)"][i]:,.2f}' for i in range(len(df_complex))],
            textposition='inside',
        ...
        fig_complex.add_trace(go.Bar(
            x=[f'Year {df_complex["Year"][i]}' for i in range(len(df_complex))],
            y=[df_complex["20% Profit Share Above 8% (₹)"][i] for i in range(len(df_complex))],
            name='20% Profit Share Above 8%',
            text=[f'₹{df_complex["20% Profit Share Above 8% (₹)"][i]:,.2f}' for i in range(len(df_complex))],
            textposition='inside',
    ...
    for i in years:
        ...
        # Calculate 30% profit share only if the total capital exceeds the threshold
        if total_capital > threshold:
            profit_share = 0.3 * (total_capital - threshold)
        else:
            profit_share = 0

        net_capital = total_capital - profit_share
    ...
    df_simple = pd.DataFrame({
        ...
        '30% Profit Share Above Threshold (₹)': profit_shares
    })
    ...
    fig_simple.add_trace(go.Bar(
        ...
        text=[f'₹{df_simple["Net Capital After Profit Share"][i]:,.2f}' for i in range(len(df_simple))],
        textposition='outside',
        marker=dict(color='#90EE90'),
        showlegend=True
    ))
    ...
    fig_simple.add_trace(go.Bar(
        ...
        text=[f'₹{df_simple["30% Profit Share Above Threshold (₹)"][i]:,.2f}' for i in range(len(df_simple))],
        textposition='inside',
        marker=dict(color='#1E90FF'),
        showlegend=True
    ))
    ...
    <div style="background-color: #F0F4F8; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 15px rgba(0, 0, 0, 0.1);">
        ...
        <ul style="list-style-type: none; padding-left: 0; font-size: 16px; color: #1E2D39;">
            ...
            <strong>Fees and Charges:</strong> For <strong>Model 1</strong>, the management fees are 5% annually for capital amounts up to ₹300,000, and 4% annually for amounts exceeding ₹300,000, with an additional 20% performance fee applied to returns exceeding 8% per year.\nIn <strong>Model 2</strong>, a 30% performance fee is applied to returns above a specified threshold.</li>
    ...
    # Additional graphs for visual representation of the fee structure and portfolio growth can be added below
    # Example: Fees Structure and Portfolio Growth Visualizations (Bar Chart, Line Chart, etc.)

elif page == "Resources & Contact":

    # Section Title with Icon and Styled Heading
    st.markdown('''
    <div style="text-align: center; padding: 20px 0;">
        <img src="https://img.icons8.com/ios-filled/50/1E2D39/contact-card.png" width="50"/>
    ...
    st.markdown('''
    <div style="background-color: #F7F7F7; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 15px rgba(0, 0, 0, 0.1);">
        <h3 style="color: #1E2D39;">Get in Touch</h3>
        <p style="font-size: 16px; color: #333333;">We would love to hear from you! Reach out to us for any inquiries, support, or feedback.</p>
        <div style="display: flex; justify-content: space-between; margin-top: 20px;">
            <div style="width: 45%; text-align: center;">
                <img src="https://img.icons8.com/ios-filled/100/007ACC/phone.png" width="50"/>
                <p style="font-size: 18px; color: #007ACC;"><strong>Phone:</strong></p>
                <p style="font-size: 18px; color: #333333;">+91-8178611382</p>
            </div>
            <div style="width: 45%; text-align: center;">
                <img src="https://img.icons8.com/ios-filled/100/007ACC/email.png" width="50"/>
                <p style="font-size: 18px; color: #007ACC;"><strong>Email:</strong></p>
